...
public class BuyerSellerTicker extends Agent {
	...
	protected void takeDown(){
		//stop advertising agent as available
		try{
	...
	public class SynchAgentsBehaviour extends Behaviour{
		...
		private int step = 0;
		private int numFinRecieved = 0; // finished messages from other agents
		private int day = 0;
		...
		public void action(){
			...
			case 0:
				...
				break;
			case 1:
				//wait to recieve a "done" message fomr all agents
				MessageTemplate mt = MessageTemplate.MatchContent("done");
				ACLMessage msg = myAgent.receive(mt);
				if(msg != null)
				{
					numFinRecieved++;
					if (numFinRecieved >= simulationAgents.size()){
						step++;
					}
				}
				else{
